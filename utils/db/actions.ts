import { db } from './dbConfiq';
import { Users, Reports, Rewards, CollectedWaste, Notifications, Transactions } from './schema';
...
export async function getCollectedWastesByCollector(collectorId: number) {
  try {
    return await db.select().from(CollectedWaste).where(eq(CollectedWaste.collectorId, collectorId)).execute();
  } catch (error) {
